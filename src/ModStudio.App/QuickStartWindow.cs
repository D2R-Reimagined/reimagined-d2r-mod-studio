...
public sealed class QuickStartWindow : Window
{
    ...
    [
        ...
        ("Find anything across the project",
         "Press Ctrl+Shift+F (or use Find in files in the toolbar) to search every table cell, string and text file at once. Results update as you type; the selected match previews below as an editable table row or as text at the matching line.\n\nEdit right in the preview and the change goes to that file's unsaved buffer, without switching tabs. Press Enter to jump to the match in the editor; drag the splitter or resize the window to see more.",
         "Match case, whole words and regular expressions are the toggles beside the search box. Narrow the scope to tables, strings or data files, or use a file mask such as *.json."),
