...
/// <summary>One affix in a pool: its levels, weight and share of the roll, and what it grants.</summary>
public sealed record AffixLine(string Name, string Levels, string Required, string Frequency, string Chance, string Group, string Effect);
...
public sealed class AffixPreviewResolver
{
    ...
    private static string Effects(PreviewTables.Session data, JsonObject affix, List<string> issues) => string.Join(" · ",
        Enumerable.Range(1, 3).Where(i => affix.S($"mod{i}code").Length > 0)
            .Select(i => PropertyText.Describe(data, affix.S($"mod{i}code"), affix.S($"mod{i}param"), affix.S($"mod{i}min"), affix.S($"mod{i}max"), issues)));
    ...
    private static AffixPreviewResult Pool(ItemCatalog catalog, JsonObject item, AffixPreviewOptions options, List<string> issues, CancellationToken token)
    {
        ...
        {
            $"{catalog.ItemName(code)} ({code}) · {string.Join(" → ", catalog.ItemTypes(item).Select(catalog.TypeName))}",
            $"qlvl {qlvl}" + (magicLevel > 0 ? $" · magic lvl {magicLevel}" : "") + (catalog.ItemClass(item) is { Length: > 0 } cls ? $" · {cls} only" : ""),
        ...
        AffixLine[] List(string table, out string locked)
        {
            ...
            locked = later.Count == 0 ? "" : $"{later.Count} more need a higher affix level; next at {next!.Key}: {string.Join(", ", next.Select(a => Name(data, a)).Distinct().Take(5))}";
            return fits.Select(a => (Row: a, Line: new AffixLine(Name(data, a), Levels(a), a.S("levelreq"), a.S("frequency"),
                    Percent(DropCalculator.Int(a, "frequency") / total), a.S("group"), Effects(data, a, issues))))
        ...
        if (item.S("auto prefix") is { Length: > 0 } group && group != "0")
        {
            ...
            sections.Add(new($"Automagic group {group} (added to every copy)", auto.Length == 0 ? ["No automagic row of this group fits at this affix level."]
                : [.. auto.Select(a => $"{Percent(DropCalculator.Int(a, "frequency") / total),6}  {Effects(data, a, issues)}  (level {Levels(a)})")]));
        }
    ...
    private static AffixPreviewResult Affix(ItemCatalog catalog, string table, JsonObject affix, List<string> issues, CancellationToken token)
    {
        ...
        var lines = new List<string>
        {
            $"{kind}" + (id >= 0 ? $" #{id} (cube pre=/suf= number)" : "") + $" · affix level {Levels(affix)} · required level {affix.S("levelreq", "0")}" +
                (affix.S("classspecific") is { Length: > 0 } cls ? $" · {cls} items only" : "") + (affix.S("class") is { Length: > 0 } levelClass ? $" · {levelClass} needs level {affix.S("classlevelreq", "?")}" : ""),
            $"Frequency {affix.S("frequency", "0")} · group {affix.S("group", "none")} · " + (Flag(affix, "rare") ? "magic and rare items" : "magic items only") + (affix.S("spawnable") == "1" ? "" : " · not spawnable")
        };
        ...
        var sections = new List<PreviewSection> { new("Effects", [.. Enumerable.Range(1, 3).Where(i => affix.S($"mod{i}code").Length > 0)
            .Select(i => PropertyText.Describe(data, affix.S($"mod{i}code"), affix.S($"mod{i}param"), affix.S($"mod{i}min"), affix.S($"mod{i}max"), issues))]) };
        var itypes = Enumerable.Range(1, 7).Select(i => affix.S("itype" + i)).Where(t => t.Length > 0).ToArray();
        ...
        var allowed = "On " + string.Join(", ", itypes.Select(t => $"{catalog.TypeName(t)} ({t})")) + (etypes.Length > 0 ? "; not on " + string.Join(", ", etypes.Select(t => $"{catalog.TypeName(t)} ({t})")) : "");
        ...
        foreach (var (_, item) in catalog.Items)
        {
            ...
            bases.Add((DropCalculator.Int(item, "level"), $"{catalog.ItemName(item.S("code"))} ({item.S("code")}) · qlvl {item.S("level", "0")} · {range}"));
        }
        ...
        {
            var peers = rows.Where(r => r.S("group") == group && !ReferenceEquals(r, affix) && !JsonNode.DeepEquals(r, affix)).Select(r => $"{Name(data, r)} ({Levels(r)})").Distinct().ToArray();
