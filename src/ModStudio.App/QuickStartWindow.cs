...
public sealed class QuickStartWindow : Window
{
    ...
    [
        ...
        ("Edit a row without horizontal scrolling",
         "Select a row, then switch the inspector to Row Editor. Every field is listed vertically. Changes apply to the shared document immediately; Save all writes them to disk.\n\nDetails lets you edit an individual cell, inspect references and create supported profile overrides.\n\nIdentity fields and locked fields are protected. Use Undo/Redo to reverse edits. Hover the toolbar icons to see their actions.",
         "Right-click rows to freeze, lock, copy or paste. Freezing keeps rows visible; locking prevents edits. Both are temporary editor settings."),
        ("Collaborate using source files",
         ...
         "Use Changes to compare the active table with its saved file. Resolve source validation errors in Problems before building."),
        ("Build, deploy, and play",
         "Choose a profile in the top-right selector. Shared edits apply across profiles; profile overrides capture intentional differences.\n\nBuild validates the source and generates a native snapshot. Deploy copies the built mod to the destination configured in Run settings. Play builds and deploys before starting the configured game or runner.\n\nKeep source and deployment directories separate. Edit the source project, then rebuild; changes made directly to deployed files are not imported back automatically.",
